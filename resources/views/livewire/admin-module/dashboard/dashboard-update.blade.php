    ...
    <div class="row">
                                ...
                                <div class="card-body">
                                    ...
                                    @if ($item->role == 'user')
                                        <a href="" data-bs-placement="top" data-bs-toggle="tooltip"
                                            ...
                                            data-bs-original-title="Change Use role to User" aria-label="Info"
                                            wire:click.prevent="UpdateAdmin('{{ $item->id }}')">Admin</a>
                                        <a href="{{ route('global_search', $item->mobile_no) }}" data-bs-placement="top"
                                            ...
                                            data-bs-original-title="Get Insight Details" aria-label="Info">Get
                                            Details</a>
                                        <a href="{{ route('wa.great', $item->mobile_no) }}" data-bs-placement="top"
                                            data-bs-toggle="tooltip" class="btn btn-sm btn-rounded btn-info"
                                            data-bs-original-title="Get Insight Details" aria-label="Info">Whatsapp</a>
                                    @elseif($item->role == 'admin' && Auth::user()->id != $item->id)
                                        <a href="" data-bs-placement="top" data-bs-toggle="tooltip"
                                            class="btn btn-sm btn-rounded btn-primary"
                                            data-bs-original-title="Change Use role to User" aria-label="Info"
                                            wire:click.prevent="UpdateUser('{{ $item->id }}')">User</a>
                                    @endif
        ...
        @elseif($Orders)
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">{{ $records->total() }} Orders Pending for this Month</h4 </div>
                    </div>
                </div>
                @foreach ($records as $item)
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-header d-sm-flex align-items-center justify-content-between"">
                                ...
                                <h5 class="card-title">{{ \Carbon\Carbon::parse($item->created_at)->diffForHumans() }}
                                </h5>
                                ...
                                <div class="col-md-4">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text"><small
                                                class="text-muted">{{ \Carbon\Carbon::parse($item->updated_at)->diffForHumans() }}</small>
                                        </p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-5">
                                        <div class="card-body">
                                            ...
                                            <a href="{{ route('update.cb.status', [$item->Id, $item->Client_Id, $item->Name]) }}"
                                                data-bs-placement="top" data-bs-toggle="tooltip"
                                                class="btn btn-sm btn-rounded btn-primary"
                                                data-bs-original-title="Change Use role to User"
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col-md-6">
                                            <div class="card-body">
                                                ...
                                                <h5 class="card-title">Service : {{ $item->Service }}</h5>
                                                <h5
                                                    class="card-title {{ $item->Status == 'Completed' ? 'text-success fw-bold font-size-16' : 'text-warning fw-bold font-size-16' }}">
                                                    Status : {{ $item->Status }}</h5>
                                                <h5
                                                    class="card-title {{ $item->Lead_Status == 'Hot' ? 'text-danger fw-bold font-size-18' : 'text-warning fw-bold font-size-16 me-2' }}">
                                                    Lead Status : {{ $item->Lead_Status }}</h5>
                                                ...
                                                <a href="{{ route('update.enquiry.dashboard', $item->Id) }}"
                                                    data-bs-placement="top" data-bs-toggle="tooltip"
                                                    class="btn btn-sm btn-rounded btn-primary"
                                                    data-bs-original-title="Update Status"
                                                    aria-label="Info">Status</a>
                                                <a href="{{ route('global_search', $item->Phone_No) }}"
                                                    data-bs-placement="top" data-bs-toggle="tooltip"
                                                    class="btn btn-sm btn-rounded btn-info"
                                                    data-bs-original-title="Get {{ $item->Name }} Details"
                                                    aria-label="Info">Details</a>
